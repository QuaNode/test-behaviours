...
export class ParametersAndReturnsComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder) {
    ...
    effect(() => {
      const _ = this.requestsService.clearSignal();
      ...
      const hasCache = this.requestsService.hasCachedResponse(currentApiName);
      if (currentApiName !== this.previousApiName && hasCache) {
      ...
      } else if (currentApiName !== this.previousApiName && !hasCache) {
        // Explicitly clear when switching to an uncached API
        this.response.set(null);
        this.returns.set({});
        this.returnKeys.set([]);
      }
  ...
  saveJson(index: number) {
    ...
    if (control) {
      ...
      this.behaviourService.updateParameters(this.jsonPreview);
    }
    this.hideJsonModal();
  }
  ...
  openJsonModal(index: number) {
    ...
    const control = this.parameters.at(index).get('rawValue');
    if (control) {
      this.jsonEditorValue = control.value;
    }
